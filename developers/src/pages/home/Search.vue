  ...
  <div class="search-page">
    ...
    <section class="content">
      <div class="search-bar">
        <input type="text" v-model="search" @keydown.enter="useEnterSearch"/>
...
export default {
  ...
  setup() {
    ...
    const getDocumentItem = (origin, route, locale, key, list) => {
    ...
    const getNewsOrCasesItem = (origin, info, locale, key) => {
      const list = []
    ...
    const useEnterSearch = () => {
      location.href = `/search?q=` + state.search
    }
    const useClickTab = (key) => {
      state.activeTab = key
      if (state.activeTab === 'all') {
        state.list = searchList
      } else {
        state.list = searchList.filter(item => item.key === key)
      }
    }
    ...
    if (q) {
      ...
      const {news, cases, documentation} = messages.value[locale.value]
      const list = [
        ...getNewsOrCasesItem(news, "news", locale, key),
        ...getNewsOrCasesItem(cases, "cases", locale, key),
      ]
      const docsList = []
      getDocumentItem(documentation, t("home.documentation.title") + " > ", locale, key, docsList)
      searchList = [...docsList, ...list.sort((a, b) => b.date - a.date)]
      useClickTab(state.activeTab)
    }
