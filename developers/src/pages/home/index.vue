<template>
  <div :class="['home-page', $i18n.locale]" ref="root">
    ...
    <section class="main-title">
      ...
      <ul>
        <li v-for="(item, index) in tm('home.main.info')" :key="index" v-html="item"></li>
        ...
        <div class="router-list">
          <a v-for="(item, index) in tm('home.main.button')" :key="index" :href="mainRoute[index]" :target="'_blank'" class="button">{{ item }}</a>
        </div>
      </ul>
    </section>
    ...
    <section class="latest-news">
      <h2>{{ t("news.title") }}</h2>
      <ul>
        <li v-for="(item, index) in tm('news.list').slice(0, 6)" :key="index">
          <a :href="'/news/' + item.filename" class="title">{{ item.title }}</a>
          <span class="time">{{ item.date }}</span>
        </li>
      </ul>
      <router-link class="button" to="/news" tag="a">{{ t("home.button.readmore") }}</router-link>
    </section>
