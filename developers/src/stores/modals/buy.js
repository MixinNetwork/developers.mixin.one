...
export const useBuyModalStore = defineStore('buy-app', () => {
  ...
  const debtAmount = computed(() => Math.max(0, appList.value.length - appProperty.value.count) * Number(appProperty.value.price));
  ...
  const useClickBuyButton = async (count) => {
    ...
    const trace = uuid();
    const amount = Number(appProperty.value.price) * count + debtAmount.value;
